 ...
 */
export const biggerThan = (rank1: string, rank2: string) => {
...
export const getUnicodeCard = (suit: Suit, rank: string | number) => {
    ...
    const code = base + (period * suit) + (typeof rank !== 'number' ? parseInt(rank as string) : rank)
...
const ranks = range(1, 11).map((num) => (num.toString())).concat(letters.toSpliced(1, 1))
